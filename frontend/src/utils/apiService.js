...
export const fetchLearnerComponents = async (learnerId, authCookies) => {
  ...
  const targetUrl = `https://corndel.aptem.co.uk/api/MWS.LearningProgrammes/Components?userId=${learnerId}&programmeId=null`;
  ...
  try {
    const response = await axios.get(proxyUrl + targetUrl, { headers });
    ...
    if (error.response?.status === 429) {
      throw new Error('CORS proxy rate limit exceeded. Please try again later or use a different proxy.');
    }
    throw error;
  }
};